...
# ── Costi reali Revolut (stima conservativa) ──────────────────────────────────
COSTS = {
    "SP500":  2.0,   # € per trade (spread + slippage)
    "GOLD":   2.5,
    "NASDAQ": 4.0,
    "WTI":    4.0,
}

CAPITAL      = 1000.0   # € capitale iniziale simulato
...
def simulate(signals_df, cost_eur, label):
    """
    signals_df: DataFrame con colonne entry, sl, tp, direction (1=long, -1=short)
    Ritorna un dict con le statistiche.
    """
    capital = CAPITAL
    trades  = []
    ...
    for _, row in signals_df.iterrows():
        ...
        # Risultato del trade (outcome simulato col prezzo successivo)
        # Usiamo il close della barra successiva come proxy
        next_close = row.get("next_close", entry)

        # Determina se ha toccato TP o SL
        if d == 1:   # LONG
        ...
        if hit_tp:
            ...
            # Non ha toccato nessuno dei due: chiudi al next_close
            pnl_pts = (next_close - entry) * d
            outcome = "OPEN"

        # Converti in € tramite risk_eur / dist_sl
...
def backtest_surfista():
    ...
    for i in range(1, len(df)-1):
        ...
        if cross_up and row["rsi"] > 50:
            signals.append({"entry": row["Close"], "sl": row["Close"]-1.5*row["atr"],
                            "tp": row["Close"]+3.0*row["atr"], "direction": 1,
                            "next_close": nxt["Close"], "name": row.name})
        elif cross_down and row["rsi"] < 50:
            signals.append({"entry": row["Close"], "sl": row["Close"]+1.5*row["atr"],
                            "tp": row["Close"]-3.0*row["atr"], "direction": -1,
                            "next_close": nxt["Close"], "name": row.name})
    ...
    sig_df = pd.DataFrame(signals).set_index("name")
    return simulate(sig_df, COSTS["SP500"], "🟢 SURFISTA (S&P500 1H)")


# ══════════════════════════════════════════════════════════════════════════════
# BACKTEST IL PENDOLO — Oro 1H
# ══════════════════════════════════════════════════════════════════════════════
def backtest_pendolo():
    ...
    sig_df = pd.DataFrame(signals).set_index("name")
    return simulate(sig_df, COSTS["GOLD"], "🟡 IL PENDOLO (Oro 1H)")


# ══════════════════════════════════════════════════════════════════════════════
# BACKTEST ROMPIGHIACCIO — Nasdaq 15min
# ══════════════════════════════════════════════════════════════════════════════
def backtest_rompighiaccio():
    ...
    for d in dates:
        ...
        rh = sess.iloc[:2]["High"].max()
        rl = sess.iloc[:2]["Low"].min()
        ...
        for i in range(2, len(sess)-1):
            ...
            if row["Close"] > rh+buf and row["Close"] > row["vwap"] and row["ema21"] > row["ema50"]:
                signals.append({"entry": row["Close"], "sl": row["Close"]-1.5*a,
                                "tp": row["Close"]+3.0*a, "direction": 1,
                                "next_close": nxt["Close"], "name": row.name})
                break
    ...
    sig_df = pd.DataFrame(signals).set_index("name")
    return simulate(sig_df, COSTS["NASDAQ"], "🔵 ROMPIGHIACCIO (Nasdaq 15min)")


# ══════════════════════════════════════════════════════════════════════════════
# BACKTEST BARILE CALDO — WTI 4H
# ══════════════════════════════════════════════════════════════════════════════
def backtest_barile_caldo():
    ...
    if len(df) < 50:
        print("  Dati insufficienti dopo resample")
        return None
    ...
    for i in range(1, len(df)-1):
        ...
        turned_bull = prev["st_dir"] == -1 and row["st_dir"] == 1
        turned_bear = prev["st_dir"] == 1  and row["st_dir"] == -1

        if turned_bull and row["Close"] > row["ema200"]:
            sl   = row["st"]
            risk = row["Close"] - sl
            if risk <= 0: continue
            signals.append({"entry": row["Close"], "sl": sl,
                            "tp": row["Close"]+2.0*risk, "direction": 1,
                            "next_close": nxt["Close"], "name": row.name})
        elif turned_bear and row["Close"] < row["ema200"]:
            sl   = row["st"]
            risk = sl - row["Close"]
            if risk <= 0: continue
            signals.append({"entry": row["Close"], "sl": sl,
    ...
    sig_df = pd.DataFrame(signals).set_index("name")
    return simulate(sig_df, COSTS["WTI"], "🛢️ BARILE CALDO (WTI 4H)")
...
def print_results(results):
    print("\n")
    print("═"*65)
    print("  RISULTATI BACKTEST — Capitale iniziale: €{:.0f}".format(CAPITAL))
    print("═"*65)

    header = f"{'Strategia':<28} {'Trade':>6} {'Win%':>6} {'PF':>6} {'MaxDD':>7} {'Rend%':>7} {'Cap.€':>8}"
    print(header)
    print("─"*65)
    ...
    print("─"*65)
    print("\nLegenda:")
    print("  PF    = Profit Factor (>1.3 accettabile, >1.5 buono)")
    print("  MaxDD = Drawdown massimo % (meglio sotto -25%)")
    print("  Rend% = Rendimento totale sul periodo testato")
    print("\n⚠️  Questi risultati includono spread+slippage stimati per Revolut.")
    print("⚠️  Il backtest usa la barra successiva come proxy — risultati indicativi.")
    print("⚠️  Passato non garantisce futuro. Fai sempre paper trading prima.\n")


# ── Entry point ───────────────────────────────────────────────────────────────
if __name__ == "__main__":
    print("🔍 Avvio backtest su dati storici reali...")
    print("   (può richiedere 2-5 minuti per il download dei dati)\n")

    results = [
        backtest_surfista(),
        backtest_pendolo(),
        backtest_rompighiaccio(),
        backtest_barile_caldo(),
    ]
